...
def main():
    ...
    parser.add_argument("--num_stochastic_samples", type=int, default=1)  # 5改为1
    ...
    inputs, _ = dataset.make_batch(args.batch_size)   # 修改
    #grounds = inputs    # 增加的
    '''
    if not isinstance(model, models.GroundTruthVideoPredictionModel):
        ...
        for k, v in inputs.items():
            if k != 'actions':
                inputs[k] = v[:, :model.hparams.context_frames]   # 修改
                #print(k)
                #print(v.shape)
                #grounds[k] = v[:, :]   # 增加的
                '''
    ...
    #ground_phs = {k: tf.placeholder(v.dtype, v.shape, '%s_ph' % k) for k, v in grounds.items()}   # 增加的
    ...
    while True:
        ...
        try:
            ...
            #ground_results = sess.run(grounds)  # 增加的
        except tf.errors.OutOfRangeError:
            break
        print("evaluation samples from %d to %d" % (sample_ind, sample_ind + args.batch_size))

        feed_dict = {input_ph: input_results[name] for name, input_ph in input_phs.items()}
        #feed_dict = {input_ph: input_results[name] for name, input_ph in input_phs.items()}  # 增加的
        for stochastic_sample_ind in range(args.num_stochastic_samples):
            #key_name = sess.run(feed_dict)
            #print(key_name['ground_truth_sampling_mean'])
            #print(key_name['ground_truth_sampling_mean'].shape)
            #for key_1,v in key_name:
                #print(key_1)
                #print(v.shape)
            
            input_images = sess.run(model.inputs['images'], feed_dict=feed_dict)   # 增加的
            ...
            for i, input_images_ in enumerate(input_images):
                ...
                input_images_ = (input_images_ * 255.0).astype(np.uint8)

                input_images_fname = 'gen_image_%05d_gd.gif' % (sample_ind + i)#, stochastic_sample_ind)
                save_gif(os.path.join(args.output_gif_dir, input_images_fname),
                         input_images_[:args.gif_length] if args.gif_length else input_images_, fps=args.fps)
                '''
                ...
                input_image_fname_pattern = 'gen_image_%%05d_gd_%%0%dd.png' % max(2, len(str(len(input_images_) - 1)))
                for t, input_image in enumerate(input_images_):
                    input_image_fname = input_image_fname_pattern % (sample_ind + i, t)
                    input_image = cv2.cvtColor(input_image, cv2.COLOR_RGB2BGR)
                    cv2.imwrite(os.path.join(args.output_png_dir, input_image_fname), input_image)
                '''    
